...
class EventCfg:
    ...
    push_robot = EventTerm(
        ...
        params={"velocity_range": {"x": (0.0, 0.0), "y": (0.0, 0.0), "z": (100, 200), "yaw": (0.0, 0.0), "pitch": (0.0, 0.0), "roll": (0.0, 0.0)},
                "asset_cfg": SceneEntityCfg("robot")},
    )
...
class ShadowHandTactileEnvCfg(DirectRLEnvCfg):
    ...
    action_space = 20
    observation_space = 160  # (full)
    state_space = 0
    asymmetric_obs = False
    obs_type = "full"

    viewer = ViewerCfg(
        # eye: カメラ自体の座標 (x, y, z) [メートル]
        # ロボットから少し離れた高い位置にするのが一般的です
        # eye=(2.0, 2.0, 0.3), 
        eye=(10.0, 10.0, 10.0), 
        
        # lookat: カメラが見つめる先の座標 (x, y, z) [メートル]
        # 通常は原点 (0, 0, 0) やロボットの初期位置に向けます
        # lookat=(0.0, 2.0, 0.3), 
        lookat=(0.0, 0.0, 0.0), 
    )
    ...
    # env
    observation_space_ = 156 + 17 + 128  # state observation + tactile + PointCloud embedding
    state_space = 156 + 17 + 128  # asymettric states + vision CNN embedding
    ...
    usd_list = sorted(glob.glob('/home/ubuntu/IsaacLab/asset/mix/train/*.usd', recursive=True))[:256]
    # in-hand object
    ...
    floor_cfg: RigidObjectCfg = RigidObjectCfg(
        prim_path="/World/envs/env_.*/floor",
        spawn=sim_utils.CuboidCfg(
            size=(1.8, 1.8, 0.1),  # 十分大きなサイズ (10m x 10m)
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.5, 0.5, 0.5)),
        
            # ★重要: キネマティック（プログラムで制御する物体）にする
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
            ...
            collision_props=sim_utils.CollisionPropertiesCfg(
                contact_offset=0.02, # 2cm手前から計算を始める
                rest_offset=0.001,
            ),
        ),
    ...
    force_delay_min_s: float = 300.0
    force_delay_max_s: float = 400.0

    # --- 報酬スケーリング設定 ---
    # 回転タスクの報酬 (基本)
    rew_scale_rotate: float = 1.0
    rew_height: float = 20.0
    ...
    max_consecutive_success = 1
    av_factor = 0.1
    act_moving_average = 1.0
    force_torque_obs_scale = 10.0
    penalty_torque_scale = -0.0005
    penalty_dof_vel_scale = -0.002
    total_scale = 0.05
